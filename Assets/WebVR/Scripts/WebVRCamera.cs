...
public class WebVRCamera : MonoBehaviour
{
    ...
    public Transform instructionsObject;
	...
	void Update()
	{
		...
		if (active) {
            ...
            Matrix4x4 m = cameraL.cameraToWorldMatrix;
            Vector3 p1 = m.MultiplyPoint(new Vector3(0, 0, 0.1f));
            Vector3 p2 = m.MultiplyPoint(new Vector3(0, 0, 1.0f));

            Vector3 cameraForward = (p2 - p1).normalized;
            cameraForward.y = 0.0f;
            instructionsObject.forward = cameraForward;
            instructionsObject.rotation *= Quaternion.Euler(0, 180, 0);
        } else {
			// apply left 
			cameraMain.worldToCameraMatrix = clv * sitStand.inverse * transform.worldToLocalMatrix;

            Matrix4x4 m = cameraMain.cameraToWorldMatrix;
            Vector3 p1 = m.MultiplyPoint(new Vector3(0, 0, 0.1f));
            Vector3 p2 = m.MultiplyPoint(new Vector3(0, 0, 1.0f));

            Vector3 cameraForward = (p2 - p1).normalized;
            
            gunObject.forward = (cameraForward);
            gunObject.rotation *= Quaternion.Euler(15, 0, 0);
            gunObject.position = p1 + new Vector3(0, -0.5f, 0);
